...
must somehow provide a method named C<bot-nick>.  It may either be provided by 
the class itself, by another role, or by a parent class.  C<IRCBot> does the
latter; it C<IRCBot> defines an attribute C<$!bot-nick> along with an
accessor method.
...
before.  This does not affect any other instances of the same class; only this
one instance has changed.
