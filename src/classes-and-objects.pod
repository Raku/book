...
Answer: Turning on parallelism is really easy; we just change the line
C<.perform() for @!dependencies;> into C<@!dependencies».perform()>. However,
we might run into race conditions in the case of diamond dependencies,
wherein C<Task>s C<A> starts C<B> and C<C> in parallel, and both start a
copy of C<D>, making C<D> run twice. The solution to this is the same as
with the cycle-detection in question 2 above: introducing an attribute
C<$!started>. We also don't die if we find a C<Task> that's been started
but not finished, because this time it might be due to parallelism rather
than cycles.
