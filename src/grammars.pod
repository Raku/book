...
Regexes inside a grammar do not need a scope declarator. They default to the
...
The C<TOP> rule anchors the match to the start and end of the string, so that
the whole string has to be in valid JSON format for the match to succeed. It
then either matches an C<< <array> >> or an C<< <object> >>.  Subsequent calls
are straightforward and reflect the structure in which JSON components can
appear.

Regexes can be recursive.  An C<array> contains C<value>. In turn a C<value>
...
form C<< token value:sym<thing> >>, which can read as I<< alternative of
...
every alternative.  The longest match wins.

=begin sidebar

The grammar engine can attempt to match alternatives in parallel.
...
Most of the grammar uses I<rules>, which are like tokens with the C<:sigspace>
modifier enabled.  As the JSON grammar uses the C<ws> token to find
significant whitespace, the simplest approach to enable single-line comments
is to override that token:
...
C<JSON::Tiny::Grammar>.  As subclasses inherit methods from superclasses, so
any grammar rule not present in the derived grammar will come from its base
grammar.
...
yourself, you'd have to traverse the match tree recursively to find and to
extract the interesting data.  An alternative solution exists: I<reduction
methods>, also called I<action methods>.
