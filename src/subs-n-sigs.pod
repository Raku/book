...
To do this, either assign a default value to the parameter in the signature:
...
Many subroutines cannot meaningfully work with arbitrary parameters,
...
Or to ensure that only arguments that exist as the key of some hash are
passed by doing:
...
specification of the arguments for a call and the call itself to be separated
