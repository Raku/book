...
Regular expressions are a computer science concept that uses simple
patterns to describe the format or composition of text. For instance, a
pattern might specify that the text start with a letter or end with a
digit or perhaps be composed entirely of digits. Pattern matching is the
process of applying these patterns to actual text to look for matches.
In Perl 6, although the specific syntax used to describe patterns is
different from other implementations of regular expressions such as
PCREN<B<P>erl B<C>ompatible B<R>egular B<E>xpressions> and
POSIXN<B<P>ortable B<O>perating B<S>ystem B<I>nterface for UniB<x>. See
IEEE standard 1003.1-2001>, we still call them C<regex>N<Historically,
because the term "regular expression" is a little cumbersome, most
people shorten it to C<regex> or C<regexp>. We use C<regex> to mean any
"regular expression-like pattern matching".>

An example: a common writing error is to duplicate a word by accident.
It is hard to catch such errors by re-reading your own text, but Perl
can do it for you with a little C<regex>:
...
The above snippet of code may look strange, but C<regex> is a succinct
and powerful way to locate parts of text that may be interesting or that
you want to perform some operation upon. The following sections go into
more detail about the specific C<regex> syntax used in Perl 6.
...
side. By default, whitespace (spaces, tabs, of other "invisible" characters)
inside the regex is irrelevant for matching, so writing the regex as
C<m/ perl />, C<m/perl/> or C<m/ p e rl/> all produce the exact same effect--
although the first way is probably the most readable.
...
Searching for literal strings gets boring pretty quickly and isn't
really that useful. Regex support special (also called I<metasyntactic>)
characters. The dot (C<.>) matches a single, arbitrary character:
...
The special variable C<$/> stores the I<match object> which is the result of
the most recent pattern match.  This allows you inspect the matched text and
access other interesting bits of information such where it appears in the 
original string.
...
"Word" character--a letter, digit, or an underscore.
...
To define your own custom character classes, list the appropriate
...
Inside a character class, non-word characters do not need to be escaped, and
generally lose their special meaning. So C<< /<[+.*]> / >> matches a plus sign,
a dot or an asterisk. The only exceptions to that are the backslash, square
brackets and the dash C<->, which need to be escaped with a backslash:
...
A I<quantifier> specifies how often something has to occur. A question
mark C<?> makes the preceding unit (be it a letter, a character class,
or something more complicatedN<we'll see what "more complicated" means
shortly>) optional, meaning it can either be present either zero or one
times. C<m/ho u? se/> matches either C<house> or C<hose>. You can also
write the regex as C<m/hou?se/> without any spaces, and the C<?> will
still quantify only the C<u>.
...
The C<regex { :ratchet ... }> pattern is so common that it has its own shortcut:
