...
branch of the type hierarchy.  More interestingly, the C<where undef> clause is
...
Perl 6 translates the C<where undef> part to a smart match operation.  In
explicit form, that's C<Any $d where { $d ~~ undef }>.
...
type check is narrower than the C<where undef> constraint.
