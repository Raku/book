...
Raku is a  powerful general-purpose programming language.
...
Every Raku program should begin with C<use v6;>. This line tells the compiler
which version of Raku the program expects.
...
C<'scores.txt'> is a I<string literal>. A string is a piece of text, and a string
...
corresponds to a certain C<$key> with C<%hash{$key}>N<In
Raku the sigil does not change when accessing an array or hash with
...
Raku code with an optional signature (the C<< -> $line >> part).  See
...
code can appear within curly braces, C<Array>s and C<Hash>es may be
...
=begin programlisting
    ...
    ...
    my B<@valid-players> = $file.get.words;
    ...

    for $file.lines -> $line {
        my ($pairing, $result) = $line.split(' | ');
        my ($p1, $p2)          = $pairing.split(' ');
        B< if $p1 ∉ @valid-players {>
        B<     say "Warning: '$p1' is not on our list!";>
        B< }>
        B< if $p2 ∉ @valid-players {>
        B<     say "Warning: '$p2' is not on our list!";>
        B< }>
        ...
    }
