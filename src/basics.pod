...
I<hash table>, I<dictionary>, or I<map>. You can ask a hash table for the value
that corresponds to a certain C<$key> with C<%hash{$key}>.

C<%games> stores the number of won games per player and C<%sets> the number of
won sets per player.
...
called C<undef>. The addition and incrementing operators treat C<undef> as a
...
won fewer games but more sets than somebody else.  This is quite possible,
especially at larger tournaments.
