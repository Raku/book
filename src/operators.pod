...
Now all the necessary information is in place, and the chart can print:
...
arrays C<@a> and C<@b> I<contain> the same values, if their containers are two
...
The default comparison is case sensitive. The second example makes the
comparison case insensitive by comparing not the values, but their
